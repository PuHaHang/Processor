...
class RecipeRefinerPrompt:
    ...
    @staticmethod
    def get_video_recipe_prompt(language: str = "ko") -> str:
        """
...
제공 인분은 숫자만 표시해주세요.
요리 과정의 시간은 컨텐츠에서 등장한 시간의 초 단위로 표시해주세요. 실수 형으로만 표시해주세요.
만약 영상 길이가 1분 미만인 경우 각 단계의 조리 시간의 끝은 다음 단계의 조리 시간의 시작 시간으로 표시해주세요.
...
"""
    ...
    @staticmethod
    def get_audio_recipe_prompt(language: str = "ko") -> str:
        """
...
조리 시간은 분 단위로 숫자만 표시해주세요.
제공 인분은 숫자만 표시해주세요.
요리 과정의 시간은 초 단위로 표시해주세요. 실수 형으로만 표시해주세요. 추정할 수 없는 경우 경험에 따라 유추해주세요.
[문자열]은 주어진 컨텐츠의 언어에 관계없이 언어 코드:{language}에 해당하는 언어로 작성해주세요.
이미지 생성 프롬프트는 요리 과정 전체에 대한 결과물을 생성하는 프롬프트로 작성해주세요. 
이미지를 최대한 사실적으로 묘사할 수 있도록 프롬프트를 작성해주세요.
이미지 생성 프롬프트는 주어진 컨텐츠의 언어에 관계없이 영어로 작성해주세요.
  ...
  "stages": [
    {
      ...
      "start_time": "18.559",
      "end_time": "70.320",
      "description": "소고기 안심 1.2kg에 소금과 후추를 골고루 뿌려 밑간한다. 아주 뜨겁게 달군 팬에 올리브 오일 2큰술를 두른 후, 밑간한 소고기 안심을 넣고 모든 면이 갈색이 될 때까지 빠르게 지진다. (소고기를 익히는 것이 아니라 표면을 캐러멜화하여 풍미를 더하는 과정이다.)"
    },
    {
      "step": 2,
      "start_time": "70.320",
      "end_time": "97.279",
      "description": "팬에서 꺼낸 뜨거운 소고기 안심의 모든 표면에 잉글리시 머스타드 2큰술를 골고루 바른다. 잠시 그대로 두어 소고기 안심이 식으면서 머스타드의 풍미를 흡수하고 육즙이 재분배되도록 한다."
    },
    {
      "step": 3,
      "start_time": "101.440",
      "end_time": "120.000",
      "description": "믹서에 밤버섯 700g, 마늘 1쪽, 소금과 후추를 넣고 곱게 간다. 여기에 소금 1큰술을 손으로 부숴 넣고 다시 한번 가볍게 간다."
    },
    {
      "step": 4,
      "start_time": "141.100",
      "end_time": "174.179",
      "description": "뜨겁게 달군 마른 팬에 간 밤버섯과 소금 혼합물(둑셀)을 넣고 수분이 완전히 날아갈 때까지 볶는다. (이 과정은 웰링턴의 성공에 매우 중요하며, 버섯의 풍미를 응축시킨다.) 마지막으로 후추를 넣고 섞은 후, 팬에서 꺼내 식힌다."
    },
    {
      "step": 5,
      "start_time": "179.220",
      "end_time": "295.739",
      "description": "랩을 넓게 펼친 후, 그 위에 삶은 밤 200g을 약간 겹치도록 평평하게 깔아 직사각형 모양을 만든다. 소금과 후추를 약간 뿌리고, 식힌 둑셀을 삶은 밤 위에 얇게 펴 바른다. 중앙에 소고기 안심을 올리고, 삶은 밤으로 소고기 안심을 감싸듯 조심스럽게 감싼다. 랩으로 단단하게 감싸서 원통형 모양을 만든 후, 양 끝을 비틀어 고정한다."
    },
    {
      "step": 6,
      "start_time": "299.059",
      "end_time": "309.260",
      "description": "단계 5에서 준비한 소고기 안심을 랩으로 감싼 상태 그대로 냉장고에 넣어 최소 15분간 굳힌다."
    },
    {
      "step": 7,
      "start_time": "311.459",
      "end_time": "365.660",
      "description": "냉장고에서 꺼낸 소고기 안심을 랩에서 벗긴다. 랩을 넓게 깐 후 그 위에 신선한 타임 1작은술을 올리고, 소고기 안심을 신선한 타임으로 감싼다. 이음새와 양 끝을 잘 봉하고 여분의 페이스트리는 잘라낸다. 다시 랩으로 단단하게 감싸서 모양을 고친다.
    },
    {
      "step": 8,
      "start_time": "374.619",
      "end_time": "401.140",
      "description": "냉장고에서 꺼낸 소고기 안심을 랩에서 벗긴 후, 파르마 햄 10장를 골고
    },
    {
      "step": 9,
      "start_time": "401.140",
