...
class RecipeBaseContentRepository:
    """RecipeBaseContent 모델 기본 CRUD 작업을 담당하는 Repository"""
    ...
    def find_by_id(self, session: Session, recipe_base_content_id: int) -> Optional[RecipeBaseContent]:
        ...
        """
        try:
            content = session.query(RecipeBaseContent).options(
                joinedload(RecipeBaseContent.recipe_base)
            ).filter(RecipeBaseContent.recipe_base_content_id == recipe_base_content_id).first()
    ...
    def find_by_recipe_base_id(self, session: Session, recipe_base_id: int) -> List[RecipeBaseContent]:
        ...
        """
        try:
            contents = session.query(RecipeBaseContent).options(
                joinedload(RecipeBaseContent.recipe_base)
            ).filter(
                RecipeBaseContent.recipe_base_id == recipe_base_id
            ).all()
            
            logger.debug(f"Found {len(contents)} recipe base contents for recipe base: {recipe_base_id}")
            return contents
    ...
    def find_by_language(self, session: Session, language: RecipeLanguage, limit: int = 100, offset: int = 0) -> List[RecipeBaseContent]:
        ...
        try:
            ...
            logger.debug(f"Found {len(contents)} recipe base contents for language: {language}")
            return contents
    ...
    def find_by_model_name(self, session: Session, model_name: str, limit: int = 100, offset: int = 0) -> List[RecipeBaseContent]:
        ...
        try:
            contents = session.query(RecipeBaseContent).options(
                ...
                RecipeBaseContent.model_name == model_name
            ).offset(offset).limit(limit).all()
            
            logger.debug(f"Found {len(contents)} recipe base contents for model: {model_name}")
            return contents
    ...
    def search_by_title(self, session: Session, title_pattern: str, limit: int = 100, offset: int = 0) -> List[RecipeBaseContent]:
        ...
        try:
            contents = session.query(RecipeBaseContent).options(
                ...
                RecipeBaseContent.title.ilike(f"%{title_pattern}%")
            ).offset(offset).limit(limit).all()
            
            logger.debug(f"Found {len(contents)} recipe base contents for title pattern: {title_pattern}")
            return contents
    ...
    def search_by_author(self, session: Session, author_pattern: str, limit: int = 100, offset: int = 0) -> List[RecipeBaseContent]:
        ...
        try:
            contents = session.query(RecipeBaseContent).options(
                ...
                RecipeBaseContent.author.ilike(f"%{author_pattern}%")
            ).offset(offset).limit(limit).all()
            
            logger.debug(f"Found {len(contents)} recipe base contents for author pattern: {author_pattern}")
            return contents
    ...
    def update(self, session: Session, recipe_base_content: RecipeBaseContent) -> RecipeBaseContent:
        ...
        """
        try:
            recipe_base_content.updated_at = datetime.utcnow()
            session.merge(recipe_base_content)
            session.flush()
    ...
    def find_all(self, session: Session, limit: int = 100, offset: int = 0) -> List[RecipeBaseContent]:
        ...
        try:
            ...
            logger.debug(f"Found {len(contents)} recipe base contents")
            return contents
    ...
    def count(self, session: Session) -> int:
        ...
        """
        try:
            count = session.query(func.count(RecipeBaseContent.recipe_base_content_id)).scalar()
