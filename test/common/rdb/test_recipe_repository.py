...
from src.common.rdb.domain.recipe.models import RecipeBase, RecipeBaseContent, RecipeBaseState, RecipeState, Recipe, Ingredient, RecipeLanguage, RecipeDifficulty, RecipeState
...
def generate_unique_checksum():
    """고유한 체크섬을 생성합니다."""
    return f"test_checksum_{int(time.time() * 1000)}_{uuid.uuid4().hex[:8]}"
...
class TestRecipeBaseRepository:
    ...
    def test_create_recipe_base(self, session, clean_database):
        ...
        recipe_base = RecipeBase(
            checksum=generate_unique_checksum(),
        ...
        # 결과 확인
        assert created_recipe_base.checksum == recipe_base.checksum
    ...
    def test_find_by_id(self, session, sample_recipe_base, clean_database):
        ...
        assert found_recipe_base.checksum == sample_recipe_base.checksum
    ...
    def test_find_by_checksum(self, session, sample_recipe_base, clean_database):
        ...
        # 체크섬으로 조회
        found_recipe_base = repository.find_by_checksum(session, sample_recipe_base.checksum)
    ...
    def test_increment_view_count(self, session, sample_recipe_base, clean_database):
        ...
        initial_count = sample_recipe_base.view_count
        
        # 조회수 증가
        repository.increment_view_count(session, sample_recipe_base)
        
        # 결과 확인
        updated_recipe_base = repository.find_by_id(session, sample_recipe_base.recipe_base_id)
        assert updated_recipe_base.view_count == initial_count + 1
    ...
    def test_exists_by_checksum(self, session, sample_recipe_base, clean_database):
        ...
        # 존재하는 체크섬 확인
        exists = repository.exists_by_checksum(session, sample_recipe_base.checksum)
        assert exists is True
        
        # 존재하지 않는 체크섬 확인
        exists = repository.exists_by_checksum(session, "non_existent_checksum")
...
class TestRecipeBaseStateRepository:
    ...
    def test_create_recipe_base_status(self, session, sample_recipe_base, clean_database):
        ...
        state = RecipeBaseState(
            recipe_base_content_id=recipe_base_content.recipe_base_content_id,
            state=RecipeState.PROCESSING
        )
        
        created_state = repository.create(session, state)
        
        # 결과 확인
        assert created_state.recipe_base_content_id == recipe_base_content.recipe_base_content_id
        assert created_state.state == RecipeState.PROCESSING
    
    def test_find_by_recipe_base_id(self, session, sample_recipe_base_state, clean_database):
        """레시피 베이스 ID로 상태 조회 테스트"""
        repository = RecipeBaseStateRepository()
        
        # 레시피 베이스 ID로 조회
        found_state = repository.find_by_recipe_base_content_id(session, sample_recipe_base_state.recipe_base_content_id)
        
        # 결과 확인
        assert found_state is not None
        assert found_state.recipe_base_content_id == sample_recipe_base_state.recipe_base_content_id
        assert found_state.state == sample_recipe_base_state.state
    
    def test_find_by_state(self, session, clean_database):
        ...
        for i, state in enumerate(states):
            recipe_base = RecipeBase(checksum=generate_unique_checksum())
            ...
            repository.create(session, state)
    ...
    def test_update_status(self, session, sample_recipe_base_state, clean_database):
        """상태 업데이트 테스트"""
        repository = RecipeBaseStateRepository()
        
        # 상태 변경
        sample_recipe_base_state.state = RecipeState.FAILED
        
        # 업데이트 실행
        updated_status = repository.update(session, sample_recipe_base_state)
        
        # 결과 확인
        assert updated_status.state == RecipeState.FAILED
    
    def test_delete_status(self, session, sample_recipe_base_state, clean_database):
        """상태 삭제 테스트"""
        repository = RecipeBaseStateRepository()
        
        # 상태 삭제
        result = repository.delete(session, sample_recipe_base_state)
    ...
    def test_count_by_state(self, session, clean_database):
...
class TestRecipeRepository:
    ...
    def test_create_recipe(self, session, complete_user, complete_recipe_base, clean_database):
        ...
        assert created_recipe.recipe_base_content_id == recipe_base_content.recipe_base_content_id
...
class TestIngredientRepository:
    ...
    def test_find_by_name(self, session, sample_ingredient, clean_database):
        ...
        assert found_ingredient.ingredient_id == sample_ingredient.ingredient_id
    ...
    def test_delete_ingredient(self, session, sample_ingredient, clean_database):
        ...
        # 삭제 확인
        found_ingredient = repository.find_by_id(session, sample_ingredient.ingredient_id)
    ...
    def test_bulk_create_or_get(self, session, clean_database):
        ...
        # 기존 재료는 기존 ID 유지
        assert ingredient_dict["기존 재료"].ingredient_id == existing_ingredient.ingredient_id
        
        # 새로운 재료들은 새로운 ID
        assert ingredient_dict["새로운 재료1"].ingredient_id != existing_ingredient.ingredient_id
        assert ingredient_dict["새로운 재료2"].ingredient_id != existing_ingredient.ingredient_id
