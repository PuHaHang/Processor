...
from src.common.rdb.domain.recipe.models import RecipeBase, RecipeBaseContent, RecipeState, Recipe, Ingredient, RecipeLanguage, RecipeDifficulty, RecipeState
...
class TestRecipeService:
    ...
    def test_create_recipe(self, session, complete_user, complete_recipe_base_content, clean_database):
        """레시피 생성 테스트"""
        ...
        assert created_recipe.recipe_base_content_id == complete_recipe_base_content.recipe_base_content_id
    ...
    def test_create_recipe_without_user(self, session, complete_recipe_base_content, clean_database):
    ...
    def test_create_recipe_without_recipe_base(self, session, complete_user, complete_recipe_base_content, clean_database):
        ...
        create_dto = CreateRecipeDto(
            user_id=complete_user.user_id,
            recipe_base_content_id=99999,
            title="테스트 레시피"
        )
        
        # 에러 발생 확인
        with pytest.raises(ValueError, match="Recipe base content not found"):
            service.create_recipe(session, create_dto)
    ...
    def test_get_recipe_by_id(self, session, sample_recipe, clean_database):
        ...
        assert found_recipe.recipe_base_content_id == sample_recipe.recipe_base_content_id
        assert found_recipe.title == sample_recipe.title
        
        # 관련 정보 로드 확인
        assert found_recipe.user is not None
        assert found_recipe.recipe_base_content is not None
    ...
    def test_get_recipes_by_user(self, session, complete_user, complete_recipe_base_content, clean_database):
        """사용자별 레시피 조회 테스트"""
        service = RecipeService()
        ...
        for recipe in user_recipes:
            ...
            assert recipe.recipe_base_content is not None
    
    def test_get_recipes_by_base(self, session, complete_user, complete_recipe_base_content, clean_database):
        """레시피 베이스별 레시피 조회 테스트"""
        service = RecipeService()
        ...
        # 레시피 베이스별 레시피 조회
        base_recipes = service.get_recipes_by_base(session, complete_recipe_base_content.recipe_base_content_id)
        
        # 결과 확인
        assert len(base_recipes) == 2
        for recipe in base_recipes:
            assert recipe.recipe_base_content_id == complete_recipe_base_content.recipe_base_content_id
            assert recipe.user is not None
            assert recipe.recipe_base_content is not None
    ...
    def test_copy_recipe_from_base(self, session, complete_user, complete_recipe_base_content, clean_database):
        ...
        assert copied_recipe.recipe_base_content_id == complete_recipe_base_content.recipe_base_content_id
    ...
    #         create_dto = CreateRecipeDto(
    #             user_id=complete_user.user_id,
    #             recipe_base_content_id=complete_recipe_base.recipe_base_id,
    #             title=title
    #         )
    #         service.create_recipe(session, create_dto)
...
class TestRecipeBaseService:
    ...
    def test_create_recipe_base(self, session, clean_database):
        ...
        assert created_recipe_base.reference["platform"] == "test"
        ...
        assert created_recipe_base.checksum is not None
    ...
    def test_get_recipe_base_by_id(self, session, complete_recipe_base, clean_database):
        ...
        assert found_recipe_base.checksum == complete_recipe_base.checksum
    ...
    def test_get_recipe_base_by_checksum(self, session, complete_recipe_base, clean_database):
        ...
        # 체크섬으로 조회
        found_recipe_base = service.get_recipe_base_by_checksum(session, complete_recipe_base.checksum)
    ...
    def test_get_recipe_bases_by_difficulty(self, session, clean_database):
        ...
        for i, difficulty in enumerate(difficulties):
            create_dto = CreateRecipeBaseDto(
                ...
                reference={"platform": "test", "url": f"https://example.com/recipe_{i}"}
            )
    ...
    def test_get_popular_recipe_bases(self, session, clean_database):
        ...
        for i in range(3):
            create_dto = CreateRecipeBaseDto(
                ...
                language=RecipeLanguage.ko,
                model_name="test_model",
                reference={"platform": "test", "url": f"https://example.com/popular_recipe_{i}"}
            )
            created_recipe_base = service.create_recipe_base(session, create_dto)
    ...
    def test_update_recipe_base_status(self, session, complete_recipe_base_content, clean_database):
        """레시피 베이스 상태 업데이트 테스트"""
        service = RecipeBaseService()
        
        # 상태 업데이트
        updated_status = service.update_recipe_base_state(
            session, 
            complete_recipe_base_content.recipe_base_content_id, 
            RecipeState.COMPLETED
        )
        
        # 결과 확인
        assert updated_status is not None
        assert updated_status.recipe_base_content_id == complete_recipe_base_content.recipe_base_content_id
    ...
    def test_update_recipe_base_status_new(self, session, clean_database):
        ...
        create_dto = CreateRecipeBaseDto(
            ...
            servings=4,
            estimated_time=60,
            reference={"platform": "test", "url": "https://example.com/status_test_recipe"},
            ingredients=[
                {"ingredient_name": "양파", "ingredient_amount": "1개", "ingredient_unit": "개"},
                {"ingredient_name": "마늘", "ingredient_amount": "3쪽", "ingredient_unit": "쪽"}
            ],
        ...
        # 레시피 베이스 컨텐츠 생성
        created_recipe_base_content = RecipeBaseContent(
            recipe_base_id=created_recipe_base.recipe_base_id,
            title="테스트 레시피",
            language=RecipeLanguage.ko,
            model_name="test_model"
        )
        session.add(created_recipe_base_content)
        session.flush()

        # 새로운 상태 생성
        new_status = service.update_recipe_base_state(
            session, 
            created_recipe_base_content.recipe_base_content_id, 
            RecipeState.PROCESSING
        )
        
        # 결과 확인
        assert new_status is not None
        assert new_status.recipe_base_content_id == created_recipe_base_content.recipe_base_content_id
    ...
    #             language=RecipeLanguage.ko,
    #             model_name="test_model",
    #             reference={"platform": "test", "url": f"https://example.com/performance_recipe_{i}"}
    #         )
    #         service.create_recipe_base(session, create_dto)
