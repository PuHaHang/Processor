...
class TestUserService:
    ...
    def test_create_user(self, session, clean_database):
        ...
        assert created_user.alert.is_alerted is True
    ...
    def test_create_guest_user(self, session, clean_database):
        ...
        # GUEST 사용자는 설정한 닉네임 사용
        assert created_user.profile is not None
        assert created_user.profile.nickname == "guest_user"
    ...
    def test_get_user_by_id_not_found(self, session, clean_database):
        ...
        # 존재하지 않는 사용자 조회
        found_user = service.get_user_by_id(session, str(ULID()))
    ...
    def test_delete_user_not_found(self, session, clean_database):
        ...
        # 존재하지 않는 사용자 삭제
        result = service.delete_user(session, str(ULID()))
    ...
    def test_update_user(self, session, complete_user, clean_database):
        ...
        assert updated_user.alert.is_alerted is False
    ...
    def test_get_user_statistics(self, session, clean_database):
        ...
        for provider, billing, region, role in test_users:
            create_dto = CreateUserDto(
                nickname=f"user_{provider.value}_{str(ULID())[-6:]}",
                role=role,
                provider=provider,
    ...
    def test_update_fcm_token_new_alert(self, session, sample_user, clean_database):
        ...
        assert updated_user is not None
        assert updated_user.alert.fcm_token == "new_fcm_token"
        assert updated_user.alert.is_alerted is False  # 기본값
    ...
    def test_update_fcm_token_not_found(self, session, clean_database):
        ...
        updated_user = service.update_fcm_token(session, str(ULID()), "new_fcm_token")
        
        # 결과 확인
        assert updated_user is None
    
    def test_update_is_alerted(self, session, complete_user, clean_database):
        """알림 플래그 업데이트 테스트"""
        service = UserService()
        
        # 알림 플래그 업데이트
        updated_user = service.update_is_alerted(session, complete_user.user_id, False)
        
        # 결과 확인
        assert updated_user is not None
        assert updated_user.alert.is_alerted is False
    
    def test_update_is_alerted_new_alert(self, session, sample_user, clean_database):
        """알림 정보가 없는 사용자의 알림 플래그 업데이트 테스트"""
        service = UserService()
        
        # 알림 플래그 업데이트 (알림 정보 없음)
        updated_user = service.update_is_alerted(session, sample_user.user_id, True)
        
        # 결과 확인
        assert updated_user is not None
        assert updated_user.alert.is_alerted is True
        assert updated_user.alert.fcm_token is None  # 기본값
    
    def test_update_is_alerted_not_found(self, session, clean_database):
        """존재하지 않는 사용자의 알림 플래그 업데이트 테스트"""
        service = UserService()
        
        # 존재하지 않는 사용자의 알림 플래그 업데이트
        updated_user = service.update_is_alerted(session, str(ULID()), True)
